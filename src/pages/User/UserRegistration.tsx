...
import type { UserQueryParams, Users } from '../../ts/api/eins/user/get_user_detail/get_user_detail_response';
import UserSearchModal from '../../components/UserSearchModal';
import { useGetUserDetails } from '../../hooks/eins/user/useGetUserDetails';
import FullScreenLoader from '../../components/FullScreenLoader';
import { useUpdateUser } from '../../hooks/eins/user/useUpdateUser';
import { useRemoveUserImage } from '../../hooks/eins/user/useRemoveUserImage';
...
function UserRegistration() {
    ...
    // User Search modal
    const [showUserSearch, setUserSearchOpen] = useState(false);
    ...
    const { data: users, isLoading, isError, refetch: refetchUsers } = useGetUserDetails(appliedFilters);

    const handleSearch = () => {
        setAppliedFilters({
            ...(filters.bioUserId && { bioUserId: filters.bioUserId }),
            ...(filters.firstName && { firstName: filters.firstName }),
            ...(filters.lastName && { lastName: filters.lastName }),
        });
    };

    const toInputDateFormat = (jsonDate: string): string => {
        if (!jsonDate) return "";
    ...
    const editClick = async (user: Users) => {
        setEditId(user.userId.toString());
        setUserId(user.bioUserId.toString());
        ...
        setUserType(user.userType ? "1" : "0");
        if (user.gender)
            setGender(user.gender);
        setActivationDate(toInputDateFormat(user.startTime));
        setExpiryDate(toInputDateFormat(user.endTime));
        try {
            const API_URL = window.__APP_CONFIG__?.API_URL;
            if (user.filePath) {
                const imageFile = await uriToFile(`${API_URL}/${user.filePath}`, user.fileName);
                if (imageFile) {
                    setPreviewSource('webcam');
                    setPreview(`${API_URL}/${user.filePath}`);
                    setSelectedFile(imageFile);
                    setShowWebcam(false);
                    setEditHaveImage(true);
                }
            } else {
                setEditHaveImage(false);
                handleRemove();
            }
        } catch (error: any) {
            console.log(error);
            console.log('Error occured creating a file from filepath')
        }
    ...
    const handleReset = () => {
        setFilters({ bioUserId: "", firstName: "", lastName: "" });
        setAppliedFilters({});
    };

    // User Search Modal end

    // Input Text Field related work
    const [editId, setEditId] = useState<string>("");
    const [editHaveImage, setEditHaveImage] = useState<boolean>(false);
    ...
    const { mutateAsync: updateUserM } = useUpdateUser();

    const { mutateAsync: removeUserImageM } = useRemoveUserImage();
    ...
    const onSaveEvent = async () => {
        ...
        try {
            ...
            if (editId == "") {
                ...
                useridAPI = saveUser.userId.toString();
            } else {
                const updateUser = await updateUserM({
                    userId: editId, payload: {
                ...
                useridAPI = updateUser.userId.toString();
            }
            ...
            if (selectedFile) {
                const userId = useridAPI;
                const image = selectedFile;
                const check = await uploadImage({
                    userId,
                    image
                });
            } else if (editId != "" && editHaveImage) {
                const userId = useridAPI;
                const rm = await removeUserImageM({ userId });
            }
    ...
    const [previewSource, setPreviewSource] = useState<'upload' | 'webcam' | null>(null);
    ...
    return (
        <>
            <FullScreenLoader show={isSubmitting}></FullScreenLoader>
            <div className="user-content-screen">
                    ...
                    <div className='d-flex align-items-center'>
                        <div className='card user-count-card'>
                            {isError && (
                                ...
                                <span className='label-count-text-failed'>
                                    Fail to load the user count
                                </span>
                            )}
                            {!isError && (<span className='label-count-text'>
                                User Count
                            </span>)}

                            {isLoading ? (<div className="skeleton-box user-count-skeleton" />) : (<span className="badge bg-success ">{users?.length.toString()}</span>)}

                        </div>
                        <div className="deviceButton">
                            <button type='button' onClick={onSearchClick} className="btn-outline-soft-blue">User Search</button>
                        </div>
                    </div>
                ...
                <div className='col-12'>
                                        ...
                                        <div className='photoMainDiv'>
                                            ...
                                            {showWebcam && (
                                                <div className='webcamOverlay'>
                                                    ...
                                                    <div className='checkRow' style={{ marginTop: "10px" }}>
                                                        <button className='btn-outline-soft-red' onClick={handleCapture}>
                                                            Click Photo
                                                        </button>
                                                        <button className='btn-outline-soft-red' onClick={() => { setShowWebcam(false); setCameraError(null); }}>
                                                            Cancel
                                                        </button>
                                                    </div>
                                                </div>
                                                ...
                                                <>
                                                    {preview ? (
                                                        <div className='removePicture'>
                                                            <div className='biggerCircle' onClick={() => inputRef.current?.click()}>
                                                    ...
                                                    <div className='checkRow'>
                                                        <button className='btn-outline-soft-red' onClick={() => inputRef.current?.click()}>
                                                            Upload
                                                        </button>
                                                        <button className='btn-outline-soft-red' onClick={() => { setShowWebcam(true); setCameraError(null); }}>
                                                            Capture
                                                        </button>
                                                    </div>
                                                </>
                                        ...
                                        <div className='row gy-2'>
                                            ...
                                            <div className='col-12 col-md-6'>
                                                <fieldset className='text-input-group'>
                                                    <label htmlFor="biouserid">User Id <span>*</span></label>
                                                    <input
                                                        id='biouserid'
                                                        type='text'
                                                        value={userid}
                                            ...
                                            <div className='col-12 col-md-6'>
                                                <fieldset className='text-input-group'>
                                                    <label htmlFor="username">First Name <span>*</span></label>
                                                    <input
                                                        id='username'
                                                        value={userFirstName}
                                            ...
                                            <div className='col-12 col-md-6'>
                                                <fieldset className='text-input-group'>
                                                    ...
                                                    <input
                                                        id='lastname'
                                                        value={userLastName}
                                                        onChange={(data) => setuserLastName(data.target.value)}
                                                    />
                                                </fieldset>
                                            </div>
                                            <div className='col-12 col-md-6'>
                                            ...
                                            <div className='col-12 col-md-6'>
                                                <fieldset className='text-input-group'>
                                                    <div className='dateRow'>
                                                        <div>
                                                            <label htmlFor="activationdate">Activation Date <span>*</span></label>
                                                            <input
                                                                type='date'
                                                                id='activationdate'
                                                                value={activationDateStr}
                                                                onChange={(data) => setActivationDate(data.target.value)}
                                                            />
                                                        </div>
                                                    ...
                                                    <div className='dateRow'>
                                                        <div>
                                                            ...
                                                            <input
                                                                ...
                                                                id='expirydate'
                                                                value={expiryDateStr}
                                                                onChange={(data) => setExpiryDate(data.target.value)}
                                                            />
                                                        </div>
                                                        <i className="bi bi-calendar-date"></i>
                                                    </div>
                    ...
                    <div className='savebuttonDiv'>
                        <button
                            className='btn-outline-soft-green'
                            onClick={onSaveEvent}
                        >Save</button>
                        <button className='btn-outline-soft-red'
                            onClick={clearSavedetails}
                        >
                            Cancel
                        </button>
                    </div>
